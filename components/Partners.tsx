...
const Partners = () => {
  ...
  useEffect(() => {
    const handleResize = () => {
      if (containerRef.current) {
        const containerWidth = containerRef.current.offsetWidth;
        // Only calculate for desktop since mobile won't show the orb
        const newSize = Math.min(containerWidth * 0.4, 400);
        setOrbSize(newSize);
      }
    };
  ...
  const getPartnerPositions = () => {
    ...
    return partners.map((name, index) => {
      ...
      // Use more conservative positioning that won't cause overflow
      const left = `calc(50% - 300px + ${col * 150}px)`;
      const top = `calc(50% - 40px + ${row * 80}px)`;
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Mobile: Simple centered grid - NO ORB */}
